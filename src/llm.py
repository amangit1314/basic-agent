...
# LEARNING NOTE: The Strategy Pattern
# -------------------------------------
# Instead of hardcoding one LLM provider, we use the Strategy Pattern:
# - One interface (LLMClient) with two methods: decide_next_action + extract_notice_data
# - Multiple implementations: Gemini, OpenAI, Claude
# - The agent doesn't care which provider is used — it just calls the interface

# This lets you switch providers with a single flag:
#     python main.py https://example.com --provider openai
#     python main.py https://example.com --provider claude
#     python main.py https://example.com --provider gemini  (default)

# PROVIDER SETUP:
# - Gemini:  Set GEMINI_API_KEY in .env   (free tier available)
# - OpenAI:  Set OPENAI_API_KEY in .env   (GPT-4o-mini is cheap)
# - Claude:  Set ANTHROPIC_API_KEY in .env (Claude Haiku is cheap)
# """
...
# - Type of notice (SARFAESI, NPA, DRT, Swiss Challenge, Auction)
...
# class LLMClient:
#     """
#     Multi-provider LLM client for the agent's brain.

#     LEARNING NOTE: This is the Strategy Pattern in action.
#     The client exposes the `extract_notice_data` method,
#     but the underlying LLM provider is swappable.

#     Usage:
#         brain = LLMClient("openai")   # or "gemini" or "claude"
#         notice = brain.extract_notice_data(...)
#     """
...
#     # -------------------------------------------------------------------------
#     # Job 2: Data Extraction — "What data is in this notice?"
#     # -------------------------------------------------------------------------
...
#     ) -> NoticeData:
#         """
#         Extract structured data from notice text. Provider-agnostic.

#         Falls back to an empty NoticeData (regex fills gaps later).
#         """
...
#                     dates.append(DateInfo(label=d["label"], value=d["value"]))

#             amounts = []
#             for a in data.get("amounts", []):
#                 if isinstance(a, dict) and a.get("label") and a.get("value"):
#                     amounts.append(AmountInfo(label=a["label"], value=a["value"]))
...
#                 auction_date=data.get("auction_date"),  # Explicit field
#                 reserve_price=data.get("reserve_price"),  # Explicit field
#                 due_amount=data.get("due_amount"),  # Explicit field
#                 dates=dates,
#                 amounts=amounts,
#                 source_url=source_url,
#                 source_type=source_type,
...
#         except Exception as e:
#             logger.warning(f"{self._provider_name} extraction failed ({e}), returning partial")
#             return NoticeData(
#                 source_url=source_url,
#                 source_type=source_type,
#                 raw_text_snippet=text[:500] if text else "",
...
# # =============================================================================
# # So existing code that imports GeminiClient still works.
